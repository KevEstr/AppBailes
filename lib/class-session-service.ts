    ...
    try {
      ...
      for (const schedule of schedules) {
        ...
        if (schedule.isActive === false) {
          console.log(
            `⏭️ Saltando horario inactivo: ${this.getDayName(
              schedule.dayOfWeek
        ...
        console.log(
          `⏰ Procesando horario: ${this.getDayName(schedule.dayOfWeek)} ${
            schedule.startTime
          }-${schedule.endTime}`
        );
        ...
        for (const sessionDate of sessionDates) {
          ...
          if (existingSession) {
            console.log(
              `⚠️  Sesión ya existe para ${sessionDate.toDateString()} ${
                schedule.startTime
              }-${schedule.endTime}`
            );
            continue;
          ...
          totalSessions++;
          console.log(
            `✅ Sesión creada para ${sessionDate.toDateString()} ${
              schedule.startTime
            }-${schedule.endTime} (Estado: ${initialStatus}). ID: ${
              newSession.id
            })`
          );
        }
