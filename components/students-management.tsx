...
export function StudentsManagement() {
  ...
  const [selectedStudent, setSelectedStudent] = useState<ClassEnrollment | null>(null)
  ...
  const handleToggleStatus = async (enrollment: ClassEnrollment) => {
    try {
      ...
      if (data.success) {
        toast({
          title: "Éxito",
          description: `Estudiante ${data.enrollment.isActive ? 'activado' : 'desactivado'} correctamente`,
        })
        loadEnrollments(currentPage, searchTerm, statusFilter)
      } else {
        toast({
  ...
  const handleDeleteEnrollment = async (enrollmentId: number) => {
    if (!confirm("¿Estás seguro de que quieres eliminar esta inscripción? Esta acción no se puede deshacer.")) {
      return
    }
  ...
  const getClassTypeIcon = (type: string) => {
    return type === 'DANCE' ? <GraduationCap className="w-4 h-4" /> : <Users className="w-4 h-4" />
  }

  const getClassTypeBadge = (type: string) => {
    return type === 'DANCE' 
      ? <Badge variant="secondary" className="bg-purple-100 text-purple-800">Baile</Badge>
