...
import { TrainerAttendanceDetailModal } from "@/components/trainer-attendance-detail-modal"
...
interface MatchTrainerAttendance {
  id: number;
  status: "PRESENT" | "LATE" | "ABSENT" | "CHANGE_REQUEST";
  date: string;
  notes?: string;
...
export default function TrainerAttendanceHistory() {
  ...
  // Estado para eventos
  const [matchAttendances, setMatchAttendances] = useState<MatchTrainerAttendance[]>([]);
  const [matchAttendanceData, setMatchAttendanceData] = useState<AttendanceData[]>([]);
  const [matchUserStats, setMatchUserStats] = useState<UserStats[]>([]);
  ...
  const [selectedUserForDetail, setSelectedUserForDetail] = useState<{ id: number; email: string } | null>(null);
  const [detailModalType, setDetailModalType] = useState<"classes" | "events">("classes");
  ...
  // Cargar asistencias de eventos
  const loadMatchAttendances = async () => {
    try {
      ...
      const response = await fetch(`/api/trainer-attendance/matches?${params}`);
      const data = await response.json();
      ...
      if (data.success) {
        ...
        if (selectedStatus && selectedStatus !== "all") {
          filteredAttendances = filteredAttendances.filter(
            (att: MatchTrainerAttendance) => att.status === selectedStatus
          );
        }
        
        setMatchAttendances(filteredAttendances);
        processMatchAttendanceData(filteredAttendances);
        processMatchUserStats(filteredAttendances);
        ...
        toast({
          title: "Error",
          description: "No se pudieron cargar las asistencias de eventos",
          variant: "destructive",
        });
      }
    } catch (error) {
      console.error("Error loading match attendances:", error);
      toast({
        title: "Error",
        description: "No se pudieron cargar las asistencias de eventos",
        variant: "destructive",
  ...
  const processMatchAttendanceData = (attendances: MatchTrainerAttendance[]) => {
    const dataMap = new Map<string, { present: number; late: number; absent: number }>();
    ...
    setMatchAttendanceData(chartData);
  };

  const processMatchUserStats = (attendances: MatchTrainerAttendance[]) => {
    const userMap = new Map<number, { 
      email: string; 
      role: string; 
      present: number; 
  ...
  useEffect(() => {
    if (activeTab === "classes") {
      ...
      loadMatchAttendances();
    }
  }, [selectedTrainer, selectedStatus, dateRange, currentPage, pageLimit, activeTab]);
  ...
  const handleExportMatchExcel = async () => {
    try {
      // Validar al menos un filtro: fecha, usuario o búsqueda
      const hasDateFilter = !!(dateRange?.from || dateRange?.to);
      const hasTrainerFilter = selectedTrainer && selectedTrainer !== "all";
      ...
      const response = await fetch(`/api/trainer-attendance/matches/export-excel?${params.toString()}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
      });
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Error al exportar asistencia de eventos de entrenadores');
      }
      ...
      const contentDisposition = response.headers.get('content-disposition');
      let filename = 'asistencia_entrenadores_eventos.xlsx';
      if (contentDisposition) {
        const filenameRegex = /filename=\"(.+)\"/;
        const filenameMatch = filenameRegex.exec(contentDisposition);
        if (filenameMatch) {
      ...
      console.error('Error downloading match trainer attendance Excel file:', error);
      toast({
        title: "❌ Error",
        description: error instanceof Error ? error.message : 'Error al descargar archivo Excel',
        variant: "destructive",
  ...
  return (
    <div>
                ...
                <Card key={user.id} className="bg-gray-700/80 border border-gray-600 rounded-2xl overflow-hidden hover:shadow-lg transition-shadow">
                  <CardContent className="p-4">
                    ...
                    <div className="mt-4">
                      <Button
                        onClick={() => {
                          setSelectedUserForDetail({ id: user.id, email: user.email });
                          setDetailModalType("classes");
          ...
          <div className="flex flex-col md:flex-row md:flex-wrap items-center md:items-end gap-4">
              ...
              <Select value={selectedTrainer} onValueChange={setSelectedTrainer}>
                ...
                <SelectContent className="bg-gray-700 border-gray-600">
                  ...
                  {trainers
                    ...
                    .map((trainer) => (
                    <SelectItem key={trainer.id} value={trainer.id.toString()} className="text-white hover:bg-yellow-600">
                      👨‍🏫 {trainer.name} {trainer.user?.email ? `(${trainer.user.email})` : ''}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="w-full md:w-[220px] min-w-[200px]">
              ...
              <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                ...
                <SelectContent className="bg-gray-700 border-gray-600">
                  ...
                  <SelectItem value="PRESENT" className="text-white hover:bg-yellow-600">
                    ✅ Presente
                  </SelectItem>
                  <SelectItem value="LATE" className="text-white hover:bg-yellow-600">
                    ⏰ Tarde
                  </SelectItem>
                  <SelectItem value="ABSENT" className="text-white hover:bg-yellow-600">
                    ❌ Ausente
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="flex items-end gap-2 w-full md:w-auto md:ml-auto">
              <Button
                ...
                title={(!dateRange?.from && !dateRange?.to && selectedTrainer === 'all') ? "Seleccione al menos un filtro para exportar" : "Exportar asistencia de eventos filtrada"}
              >
                {isDownloading ? (
                  <>
      ...
      {/* Tabla de usuarios para eventos */}
      <Card className="border-0 shadow-2xl rounded-xl bg-gray-800/90 border border-gray-600 backdrop-blur-sm">
        <CardHeader className="pb-4">
          <CardTitle className="flex items-center space-x-3 text-white">
            <Trophy className="w-6 h-6" />
            <span className="text-2xl font-bold">
              Estadísticas por Usuario - Eventos
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {matchUserStats.length === 0 ? (
              <div className="text-center py-12">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="text-base font-medium text-white">{user.email}</h4>
                          <p className="text-sm text-gray-300 mt-1">
                            {user.role} • {user.totalClasses} eventos totales
                          </p>
                        </div>
                      </div>
