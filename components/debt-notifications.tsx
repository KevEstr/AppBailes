...
export function DebtNotifications() {
  ...
  const getUrgencyColor = (daysOverdue: number) => {
    if (daysOverdue >= 30)
      return "border-red-500 bg-gradient-to-r from-red-950/50 to-pink-950/50";
    if (daysOverdue >= 15)
      return "border-orange-500 bg-gradient-to-r from-orange-950/50 to-amber-950/50";
    if (daysOverdue >= 7)
      return "border-amber-500 bg-gradient-to-r from-amber-950/50 to-yellow-950/50";
  ...
  const getUrgencyBadge = (daysOverdue: number) => {
    ...
    if (daysOverdue >= 7)
      return { variant: "secondary" as const, label: "Atención" };
    ...
    return (
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <Card
            key={i}
            className="animate-pulse border-0 shadow-xl rounded-3xl bg-gray-800/90 border border-gray-600"
          >
          ...
          <CardTitle className="flex items-center space-x-4">
            ...
            <div>
              <span className="text-3xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">
                Control Pagos Paradise
              </span>
              <p className="text-blue-300 mt-2 text-lg">
                Seguimiento de mensualidades pendientes
              </p>
            </div>
          </CardTitle>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {debts.map((debt) => {
              ...
              return (
                <Card
                  key={debt.id}
                  className={`border-0 shadow-2xl transition-all duration-500 hover:shadow-3xl hover:-translate-y-1 rounded-3xl bg-gray-800/90 border border-gray-600 hover:border-red-500 backdrop-blur-sm ${getUrgencyColor(
                    debt.daysOverdue
                  )}`}
                >
                  <CardContent className="p-8">
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <h4 className="font-bold text-white flex items-center space-x-3 mb-3 text-xl">
                            ...
                            <Badge {...urgency}>{urgency.label}</Badge>
                          </h4>
