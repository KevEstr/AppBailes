...
export function AttendanceSystem() {
  ...
  const { data: classesData, error: classesError } = useParadiseApi<{
    success: boolean;
    classes: DanceClass[];
  }>("/classes?active=true");
  ...
  const loadTodaySession = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/class-sessions?classId=${selectedClass}&date=${today}`
      );
          ...
          <CardTitle className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-blue-300 mt-2 text-lg">
                Control visual de asistencias en tiempo real
              </p>
            </div>
          </CardTitle>
        </CardHeader>
          ...
          <div className="flex items-center space-x-6">
                ...
                <SelectContent className="bg-gray-700 border-gray-600">
                  {classes.map((danceClass) => (
                    <SelectItem
                      ...
                      className="h-16 py-4 text-white hover:bg-blue-600"
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-400">
                            ...
                            {danceClass.enrollments.length} estudiantes
                          </div>
                        </div>
                      </div>
                    </SelectItem>
            ...
            {currentSession && (
              <div className="text-center">
                <Badge
                  variant="outline"
                  className="bg-purple-950/50 text-purple-400 border-purple-500 text-lg px-4 py-2"
                >
      ...
      {!selectedClass ? (
        <Card className="border-0 shadow-xl rounded-2xl bg-gray-800/90 border border-gray-600 backdrop-blur-sm">
          <CardContent className="p-12 text-center space-y-4">
            ...
            <p className="text-gray-300">
              Elige la clase de baile para tomar asistencia
            </p>
          </CardContent>
        </Card>
      ) : !currentSession ? (
        <Card className="border-0 shadow-xl rounded-2xl bg-gray-800/90 border border-gray-600 backdrop-blur-sm">
          <CardContent className="p-12 text-center space-y-4">
            ...
            <p className="text-gray-300">
              No hay sesión programada para la fecha actual
            </p>
          </CardContent>
        </Card>
      ) : students.length === 0 ? (
