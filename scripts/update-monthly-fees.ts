...
interface StudentWithEnrollmentData {
  id: string;
  name: string;
  enrollmentData: {
...
async function updateMonthlyFees(): Promise<void> {
  try {
    ...
    for (const student of studentsWithEnrollmentData) {
      try {
        ...
        const primarySport: 'DANCE' | 'VOLLEYBALL' = sports.includes('DANCE') ? 'DANCE' : sports[0];
        ...
        console.error(`   ❌ Error procesando estudiante ${student.name}:`, error instanceof Error ? error.message : 'Error desconocido');
        errorCount++;
      }
    }
