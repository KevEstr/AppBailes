...
export async function POST(request: Request) {
  try {
    ...
    const metrics = await calculateFinancialMetrics(period.id, startDate, endDate);
...
async function calculateFinancialMetrics(periodId: number, startDate: Date, endDate: Date) {
  ...
  // Total de ingresos
  const totalIncome = (monthlyPaymentsIncome._sum.amount || 0) + (servicePaymentsIncome._sum.amount || 0);
  ...
  const summary = {
    ...
    averageTransactionAmount: totalIncome > 0 ? totalIncome / (await prisma.receipt.count({
      where: {
        createdAt: {
          gte: startDate,
          lte: endDate
